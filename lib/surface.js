 ...
 */
export function splitSystemHead(messages) {
...
function resultText(content) {
    ...
    if (other > 0)
        parts.push(`[${other} non-text block(s)]`);
    return parts.join('\n');
}
/**
 ...
 */
export function projectMessages(region) {
    return region.map((message) => {
        ...
        if (other > 0)
            text.push(`[${other} non-text block(s)]`);
        return {
 ...
 */
function truncatedResultText(text, headChars) {
 ...
 */
function renderMessage(message, actions, indexed, truncateHeadChars, counter) {
    const lines = [];
    let text = '';
    let other = 0;
    for (const block of message.content) {
        switch (block.type) {
            case 'text':
                ...
                break;
            case 'tool-call': {
                if (actions.get(block.id) === 'drop_call')
                    break;
            ...
            case 'tool-result': {
                ...
                if (action === 'drop_result') {
                    ...
                    if (head === original)
                        lines.push(`[tool result ${block.toolCallId} ${ok}]`, original);
                    else
                        lines.push(`[tool result ${block.toolCallId} ${ok}, ${call?.resultChars ?? original.length} chars released by Jev]`, head);
                }
                else {
 ...
 */
export function renderRegion(input) {
    ...
    for (const decision of input.decisions) {
        ...
        if (call !== undefined)
            actions.set(call.callId, decision.action);
    }
    ...
    const resultsDropped = input.decisions.filter((decision) => decision.reason === 'result_dropped').length;
    const callsDropped = input.decisions.filter((decision) => decision.reason === 'call_dropped').length;
    const ledger = [
