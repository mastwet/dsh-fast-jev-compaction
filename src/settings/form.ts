...
import {
  JEV_SETTING_FIELDS, JEV_SETTING_ORDER, type JevSettingKey, type JevSettings,
} from './fields.js'
 ...
 */
export const JEV_STAGED_FIELDS: readonly JevSettingKey[] =
  JEV_SETTING_ORDER.filter(key => JEV_SETTING_FIELDS[key].kind !== 'boolean')

/** Whether a stored layer carries its own value for one field. */
function carries(layer: unknown, field: string): boolean {
 ...
 */
export function isOverridden(user: unknown, field: JevSettingKey): boolean {
 ...
 */
export function draftFrom(value: JevSettings, user: unknown): JevDraft {
 ...
 */
export function issueOf(field: JevSettingKey, text: string): JevIssue | undefined {
  ...
  if (spec.min !== undefined && parsed < spec.min) return 'range'
  if (spec.max !== undefined && parsed > spec.max) return 'range'
  return undefined
}

/** The value staged text writes for one field, once {@link issueOf} accepts it. */
function valueOf(field: JevSettingKey, text: string): number | string {
  const trimmed = text.trim()
 ...
 */
export function planWrites(draft: JevDraft, user: unknown, value: JevSettings): JevWrite[] {
  const writes: JevWrite[] = []
  for (const field of JEV_STAGED_FIELDS) {
    const text = draft[field] ?? ''
    if (issueOf(field, text) !== undefined) continue
