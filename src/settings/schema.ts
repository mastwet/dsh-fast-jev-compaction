 ...
 */
function numberSchema(field: JevSettingField, fallback: number): z<number> {
 ...
 */
function fieldSchema<K extends JevSettingKey>(key: K): z<JevSettings[K]> {
  const field = JEV_SETTING_FIELDS[key]
  const fallback = JEV_SETTINGS_DEFAULTS[key]
  switch (field.kind) {
    case 'boolean':
      return z.boolean().default(fallback as boolean) as z<JevSettings[K]>
    case 'number':
      return numberSchema(field, fallback as number) as z<JevSettings[K]>
    case 'string':
      return z.string().default(fallback as string) as z<JevSettings[K]>
  }
}

const dict = Object.fromEntries(JEV_SETTING_ORDER.map((key) => [key, fieldSchema(key)])) as {
  [K in JevSettingKey]: z<JevSettings[K]>
}
 ...
 */
export const JevSettingsSchema = z.object(dict) as unknown as z<JevSettings>